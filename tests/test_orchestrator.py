...
from app.orchestration.aggregator import build_summary, collect_issues
...
class DocumentPipelineTests(unittest.TestCase):
    ...
    def test_logic_issues_are_not_counted_as_fixable(self) -> None:
        formatting_issue = _build_issue("issue_rag_001")
        logic_issue = Issue(
            id="issue_logic_001",
            type="logic",
            ...
            severity="warning",
            message="?????????? ???????????? ? ?????????",
            location=IssueLocation(page=3),
            evidence="? ????? ??????? ??????? ???? ????????, ? ?????? ??????.",
            standard_reference=StandardReference(source="logic_rules", rule_id="logic_rule_05", quote="? ????????? ?? ?????? ???? ?????????? ????????????."),
            suggestion="????????? ???????????? ? ????????? ???????????? ???????.",
            agent="logic_agent",
        )

        summary = build_summary([formatting_issue, logic_issue])

        self.assertEqual(summary.total_issues, 2)
        self.assertEqual(summary.fixable, 1)
        self.assertEqual(summary.by_type.get("logic"), 1)
